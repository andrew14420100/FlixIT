...
import type { Content, ContentWithTMDB, TMDBSearchResult, StreamSource } from '../types';
...
const ContentsPage: React.FC = () => {
  ...
  const openStreamDialog = async (content: ContentWithTMDB): Promise<void> => {
    ...
    try {
      const res = await adminAPI.getContentStreams(content.tmdbId);
      setStreamList(res.streams || []);
      const movieSource = (res.streams || []).find((st) => st.media_type === 'movie');
      if (content.type === 'movie' && movieSource) setStreamUrl(movieSource.stream_url);
    } catch {
      showSnackbar('Impossibile caricare gli stream configurati', 'error');
    } finally {
      setStreamLoading(false);
    }
  };

  const handleSaveStream = async (): Promise<void> => {
    if (!selectedContent) return;
    const url = streamUrl.trim();
    if (!/^https?:\/\/.+\.(m3u8|mp4|m4v|webm|mov)(\?.*)?$/i.test(url)) {
      showSnackbar('Inserisci un URL valido (.m3u8 o .mp4)', 'error');
      return;
    }
    setFormLoading(true);
    try {
      const isTv = selectedContent.type === 'tv';
      await adminAPI.setContentStream(
        selectedContent.tmdbId,
        { stream_url: url, season: isTv ? parseInt(streamSeason, 10) : null, episode: isTv ? parseInt(streamEpisode, 10) : null },
        selectedContent.type
      );
      showSnackbar('Stream salvato', 'success');
      const res = await adminAPI.getContentStreams(selectedContent.tmdbId);
      setStreamList(res.streams || []);
      if (isTv) setStreamUrl('');
    } catch (err) {
      showSnackbar((err as Error)?.message || 'Errore nel salvataggio dello stream', 'error');
    } finally {
      setFormLoading(false);
    }
  };

  const handleRemoveStream = async (source: StreamSource): Promise<void> => {
    if (!selectedContent) return;
    try {
      await adminAPI.deleteContentStream(selectedContent.tmdbId, source.season, source.episode);
      setStreamList((prev) => prev.filter((st) => !(st.season === source.season && st.episode === source.episode)));
      if (selectedContent.type === 'movie') setStreamUrl('');
      showSnackbar('Stream rimosso', 'success');
    } catch {
  ...
  return (
    <Box data-testid="contents-page">
                ...
                >
                  <Box sx={{ position: 'relative' }}>
                    ...
                    >
                      ...
                      <Tooltip title="Stream video">
                        <IconButton
                          size="small"
                          data-testid={`stream-button-${content.tmdbId}`}
                          onClick={() => openStreamDialog(content)}
                          sx={{
                            bgcolor: 'rgba(255,255,255,0.1)',
                            color: 'grey.300',
                            '&:hover': { bgcolor: 'rgba(255,255,255,0.2)' },
      ...
      >
        <DialogTitle sx={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          Stream video - {String(selectedContent?.title ?? selectedContent?.name ?? '')}
        </DialogTitle>
        <DialogContent sx={{ pt: '24px !important' }}>
          <Typography variant="body2" sx={{ color: 'grey.500', mb: 2 }}>
            Inserisci l'URL di un flusso HLS (.m3u8) o di un file MP4 di cui possiedi i diritti. Il player nativo lo
            riprodurrà al posto dell'iframe. {selectedContent?.type === 'tv' ? 'Per le serie indica stagione ed episodio.' : ''}
          </Typography>
          {selectedContent?.type === 'tv' && (
            <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
              <TextField
                ...
                type="number"
                value={streamSeason}
                onChange={(e) => setStreamSeason(e.target.value)}
                inputProps={{ min: 0, 'data-testid': 'stream-season-input' }}
                sx={{ flex: 1 }}
              />
              <TextField
                label="Episodio"
                ...
                inputProps={{ min: 1, 'data-testid': 'stream-episode-input' }}
                sx={{ flex: 1 }}
              />
            </Box>
          )}
          <TextField
            fullWidth
            label="URL stream (.m3u8 / .mp4)"
            placeholder="https://cdn.example.com/video/master.m3u8"
            value={streamUrl}
            onChange={(e) => setStreamUrl(e.target.value)}
            inputProps={{ 'data-testid': 'stream-url-input' }}
          />
          ...
          {streamLoading ? (
            ...
            <Box sx={{ mt: 3 }}>
              <Typography variant="subtitle2" sx={{ color: 'grey.400', mb: 1 }}>
                Stream configurati ({streamList.length})
              </Typography>
              {streamList.map((st) => (
                <Box
                  key={`${st.season ?? 'm'}-${st.episode ?? 'm'}`}
                  data-testid="stream-source-row"
                  sx={{
                    display: 'flex', alignItems: 'center', gap: 1.5, py: 1, px: 1.5, mb: 1,
                    bgcolor: 'rgba(255,255,255,0.04)', borderRadius: 2, border: '1px solid rgba(255,255,255,0.06)',
                  }}
                >
                  <Chip
                    size="small"
                    label={st.media_type === 'tv' ? `S${st.season}E${st.episode}` : 'Film'}
                    sx={{ bgcolor: 'rgba(229,9,20,0.2)', color: '#fff', fontWeight: 600 }}
                  />
                  <Chip size="small" label={st.type.toUpperCase()} sx={{ bgcolor: 'rgba(255,255,255,0.1)', color: 'grey.300' }} />
                  <Typography noWrap sx={{ flex: 1, color: 'grey.300', fontSize: 13 }} title={st.stream_url}>
                    {st.stream_url}
                  </Typography>
                  <Tooltip title="Rimuovi stream">
                    <IconButton size="small" onClick={() => handleRemoveStream(st)} sx={{ color: 'grey.500', '&:hover': { color: '#e50914' } }}>
            ...
            <Typography variant="body2" sx={{ color: 'grey.600', mt: 3 }}>
              Nessuno stream configurato: il player proverà il provider di pubblico dominio, altrimenti mostrerà "Stream non disponibile".
            </Typography>
          )}
        </DialogContent>
        <DialogActions sx={{ p: 3, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          <Button onClick={() => setStreamDialogOpen(false)} sx={{ color: 'grey.400' }}>
            Chiudi
          </Button>
          <Button
            onClick={handleSaveStream}
            variant="contained"
            disabled={formLoading || !streamUrl.trim()}
            data-testid="stream-save-button"
            sx={{ bgcolor: '#e50914', '&:hover': { bgcolor: '#b20710' } }}
          >
            {formLoading ? <CircularProgress size={20} /> : 'Salva stream'}
          </Button>
        </DialogActions>
      </Dialog>
