// @ts-nocheck
import type { Content, HeroSettings, Section, DashboardStats, AdminLog, PaginatedResponse, ApiError, StreamSource } from '../types';
...
class AdminAPIService {
  ...
  async getContentStreams(tmdbId: number): Promise<{ tmdbId: number; streams: StreamSource[] }> {
    return this.request<{ tmdbId: number; streams: StreamSource[] }>(`/admin/contents/${tmdbId}/streams`);
  }

  async setContentStream(
    tmdbId: number,
    data: { stream_url: string; season?: number | null; episode?: number | null },
    mediaType?: 'movie' | 'tv'
  ): Promise<{ success: boolean; stream: StreamSource }> {
    const qs = mediaType ? `?media_type=${mediaType}` : '';
    return this.request<{ success: boolean; stream: StreamSource }>(`/admin/contents/${tmdbId}/stream${qs}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    });
  }
  ...
  async deleteContentStream(
    ...
    return this.request<{ success: boolean; deleted: number }>(`/admin/contents/${tmdbId}/stream${qs}`, {
      method: 'DELETE',
    });
  }
  ...
  async getAvailableSections(): Promise<{ available: any[]; all: any[] }> {
    return this.request<{ available: any[]; all: any[] }>('/admin/available-sections');
  }

  async addPredefinedSection(name: string): Promise<any> {
    return this.request<any>('/admin/sections/add-predefined', {
      method: 'POST',
