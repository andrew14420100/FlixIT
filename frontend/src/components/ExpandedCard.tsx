  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "14px 16px 16px", opacity: entered ? 1 : 0, transition: "opacity 220ms ease 60ms" }}>
        ...
        {(isUpcoming || item.new_release) && (
          <Typography data-testid={`card-upcoming-${id}`} sx={{ fontSize: 14, fontWeight: 700, color: item.new_release ? "#fff" : "#ff2e38", letterSpacing: "0.02em", mb: 0.6, textTransform: "uppercase" }}>
            {item.badge || formatReleaseLabel(item.release_date)}
          </Typography>
        )}
        {watch && (
