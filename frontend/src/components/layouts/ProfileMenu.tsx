...
const ITEMS = [
  ...
  { id: "my-list", label: "La mia lista", hint: "Titoli salvati", Icon: BookmarkBorderIcon, to: "/account?section=list" },
  { id: "continue", label: "Continua a guardare", hint: "Riprendi da dove eri", Icon: PlayCircleOutlineIcon, to: "/account?section=watching" },
  { id: "subscription", label: "Abbonamento", hint: "Stato Premium e pagamenti", Icon: WorkspacePremiumIcon, to: "/account?section=subscription" },
];
...
function Row({ id, label, hint, Icon, onClick, color = "#fff", iconBg = "rgba(255,255,255,0.06)" }) {
  return (
    <Box component="button" type="button" onClick={onClick} data-testid={`menu-${id}`} sx={{ ...rowSx, color }}>
      <Box sx={{ width: 36, height: 36, borderRadius: "10px", display: "grid", placeItems: "center", bgcolor: iconBg, flexShrink: 0 }}>
        <Icon sx={{ fontSize: 20, color: color === "#fff" ? "rgba(255,255,255,0.85)" : color }} />
      </Box>
      <Box sx={{ flex: 1, minWidth: 0 }}>
        <Typography sx={{ fontSize: 14, fontWeight: 600, lineHeight: 1.2 }}>{label}</Typography>
        {hint && <Typography noWrap sx={{ fontSize: 12, color: "rgba(255,255,255,0.45)", mt: 0.2 }}>{hint}</Typography>}
      </Box>
  ...
  return (
    <Popover
      ...
      slotProps={{ paper: { "data-testid": "profile-menu", sx: {
        mt: 1.5, width: 310, p: 1, borderRadius: "18px", bgcolor: "rgba(12,12,14,0.92)", backdropFilter: "blur(28px) saturate(180%)",
    ...
    >
      <Box data-testid="profile-menu-header" sx={{ position: "relative", p: 2, borderRadius: "14px", overflow: "hidden",
        ...
        <Box data-testid="profile-menu-premium-status" sx={{ mt: 1.5, display: "flex", alignItems: "center", gap: 0.8, px: 1.2, py: 0.6, borderRadius: "10px", bgcolor: isPremium ? "rgba(229,9,20,0.25)" : "rgba(255,255,255,0.06)", position: "relative" }}>
          <WorkspacePremiumIcon sx={{ fontSize: 16, color: isPremium ? "#ff8a90" : "#A3A3A3" }} />
          <Typography noWrap sx={{ fontSize: 12, fontWeight: 600, color: isPremium ? "#fff" : "#A3A3A3" }}>{premiumLabel(user)}</Typography>
        </Box>
      </Box>

      <Box sx={{ mt: 1, display: "flex", flexDirection: "column", gap: 0.25 }}>
        {ITEMS.map((it) => <Row key={it.id} {...it} onClick={() => onNavigate(it.to)} />)}
        {!isPremium && user?.role !== "superadmin" && <Row id="upgrade" label="Passa a Premium" hint="Sblocca le sezioni esclusive" Icon={WorkspacePremiumIcon} color="#ff8a90" iconBg="rgba(229,9,20,0.18)" onClick={() => onNavigate("/premium")} />}
        {user?.is_admin && <Row id="admin" label="Pannello admin" hint={user.role === "superadmin" ? "Superadmin" : "Amministratore"} Icon={AdminPanelSettingsOutlinedIcon} color="#fbbf24" iconBg="rgba(251,191,36,0.14)" onClick={goAdmin} />}
      </Box>
