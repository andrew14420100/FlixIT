    ...
    >
      ...
      >
        ...
        {(video.upcoming || video.new_release) && (
          <div data-testid={`upcoming-badge-${video.id}`} style={{ position: "absolute", top: 8, left: 8, padding: "3px 8px", borderRadius: 6, background: video.new_release ? "#fff" : "#E50914", color: video.new_release ? "#000" : "#fff",
            fontSize: 11, fontWeight: 800, letterSpacing: "0.04em", textTransform: "uppercase", boxShadow: "0 4px 12px rgba(0,0,0,0.5)" }}>
            {video.badge || (video.release_date ? formatReleaseLabel(video.release_date) : "Prossimamente")}
          </div>
        )}
        {watch && (
