...
def resolve_viewer(request: Request, auth: Optional[str]):
    ...
    if payload.get("user_id"):
        u = col("users").find_one({"id": payload["user_id"]}, {"role": 1})
...
def register(app, db, get_current_user, get_current_admin, jwt_secret, log_admin_action, issue_user_token):
    ...
    def user_summary(u: dict) -> dict:
        from premium import premium_status, is_premium
        return {
            ...
            "premium": premium_status(u), "is_premium": is_premium(u),
    ...
    def list_users(q: Optional[str] = None, role: Optional[str] = None, page: int = 1, limit: int = 25, admin=Depends(get_current_admin)):
        flt = {}
        if q:
        ...
        return {"items": [user_summary(u) for u in docs], "total": total, "page": page,
                "stats": {"total": users.count_documents({}), "banned": users.count_documents({"banned": True}),
                          "premium": users.count_documents({"premium.active": True}), "admins": users.count_documents({"role": {"$in": ["admin", "superadmin"]}}),
                          "pending_reset": users.count_documents({"must_reset_password": True})}}
    ...
    def patch_user(user_id: str, data: UserPatch, admin=Depends(get_current_admin)):
        ...
        if data.role is not None:
            ...
            if (data.role == "superadmin" or u.get("role") == "superadmin") and admin.get("role") != "superadmin":
                raise HTTPException(status_code=403, detail="Solo un superadmin può gestire i superadmin")
            upd["role"] = data.role
        if data.banned is not None:
            if user_id == admin["id"]:
                raise HTTPException(status_code=400, detail="Non puoi sospendere il tuo account")
            if u.get("role") == "superadmin" and admin.get("role") != "superadmin":
                raise HTTPException(status_code=403, detail="Solo un superadmin può sospendere un superadmin")
    ...
    def delete_user(user_id: str, admin=Depends(get_current_admin)):
        ...
        target = users.find_one({"id": user_id}, {"role": 1})
        if target and target.get("role") == "superadmin" and admin.get("role") != "superadmin":
            raise HTTPException(status_code=403, detail="Solo un superadmin può eliminare un superadmin")
