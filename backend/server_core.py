# --- IMPORT LOCALI / SERVIZI ---
from services.vixsrc import resolve_vixsrc_stream
# from services.altri_servizi import ...
...
import ssl
import certifi
import player as _player  # native player: stream resolution (admin sources + public-domain fallback)
...
def _decode_token_user(credentials: HTTPAuthorizationCredentials) -> dict:
    """Decode a JWT and load the user from the single `users` collection (no email hardcoding)."""
    try:
    ...
    query = {"id": payload["user_id"]} if payload.get("user_id") else ({"email": payload.get("email", "").lower()} if payload.get("email") else None)
    if not query:
        raise HTTPException(status_code=401, detail="Invalid token")
...
def require_role(*roles):
    """Single authorization middleware: allows only users whose `role` is in `roles`."""
    def dep(credentials: HTTPAuthorizationCredentials = Depends(security)):
        user = _decode_token_user(credentials)
...
# =====================
# VIXSRC STREAMING ROUTE
# =====================

@app.get("/api/stream/vixsrc")
async def get_proxied_vixsrc_stream(
    tmdb_id: str = Query(..., description="ID TMDB del film o della serie TV"),
    season: int = Query(None, description="Numero della stagione (opzionale, per serie TV)"),
    episode: int = Query(None, description="Numero dell'episodio (opzionale, per serie TV)")
):
    """
    Risolve il flusso da VixSrc tramite il resolver e lo incapsula usando il MediaFlow Proxy configurato.
    """
    try:
        # 1. Risolvi il flusso grezzo e gli header tramite il modulo VixSrc
        vix_data = await resolve_vixsrc_stream(tmdb_id=tmdb_id, season=season, episode=episode)
        source_url = vix_data["stream_url"]
        stream_headers = vix_data["headers"]

        # 2. Recupera la configurazione di MediaFlow centralizzata (da MongoDB / app_settings)
        mf_config = _mediaflow.get_config(get_setting)
        mediaflow_url = mf_config.get("url", "https://mediaflow-proxy-deploy.onrender.com")
        api_password = mf_config.get("api_password", "")
        mediaflow_enabled = bool(get_setting(_mediaflow.ENABLED_KEY, True)) and mf_config.get("enabled", True)

        if not mediaflow_enabled:
            # Fallback: restituisce l'URL diretto se MediaFlow è disabilitato
            return {
                "success": True,
        ...
        # 3. Costruisci l'URL finale proxato usando la funzione nativa build_proxy_url
        proxied_url = _mediaflow.build_proxy_url(
        ...
        logger.error(f"Errore generazione stream VixSrc/MediaFlow per TMDB {tmdb_id}: {str(e)}")
...
class AppSettingsUpdate(BaseModel):
    ...
    public_domain_fallback: Optional[bool] = None  # native player: Internet Archive resolver toggle
    cache_ttl_hours: Optional[float] = None         # native player: resolved-stream cache TTL (hours)
    resolver_order: Optional[list] = None           # native player: provider ids in resolution order
    mediaflow_url: Optional[str] = None             # MediaFlow Proxy base URL ("" = disabled, direct streams)
    mediaflow_api_password: Optional[str] = None
    mediaflow_enabled: Optional[bool] = None
    stremio_addon_url: Optional[str] = None          # Stremio addon base URL ("" = disabled)
    stremio_enabled: Optional[bool] = None
    vixsrc_enabled: Optional[bool] = None            # VixSrc resolver on/off (primary source)


class MediaflowTest(BaseModel):
    mediaflow_url: Optional[str] = None
    mediaflow_api_password: Optional[str] = None


class StremioTest(BaseModel):
    stremio_addon_url: Optional[str] = None


def _stremio_settings() -> dict:
    cfg = _stremio.get_config(get_setting)
    return {
        "stremio_addon_url": cfg["url"],
        "stremio_enabled": bool(get_setting(_stremio.ENABLED_KEY, True)),
        "stremio_active": cfg["enabled"],
    }


def _mediaflow_settings() -> dict:
    cfg = _mediaflow.get_config(get_setting)
    return {
        "mediaflow_url": cfg["url"],
        "mediaflow_api_password": cfg["api_password"],
        "mediaflow_enabled": bool(get_setting(_mediaflow.ENABLED_KEY, True)),
        "mediaflow_active": cfg["enabled"],
    }
...
async def get_admin_settings(admin = Depends(get_current_admin)):
    ...
    return {
        ...
        "public_domain_fallback": bool(get_setting(_player.PUBLIC_DOMAIN_SETTING_KEY, False)),
        "stream_sources_count": _player.count_stream_sources(),
        "player_resolvers": _player.registry.providers_config(),
        "cache_ttl_hours": _player.registry.cache_ttl_hours(),
        "vixsrc_enabled": bool(get_setting("vixsrc_enabled", True)),
        **_mediaflow_settings(),
        **_stremio_settings(),
...
def update_admin_settings(data: AppSettingsUpdate, admin = Depends(get_current_admin)):
    ...
    if data.public_domain_fallback is not None:
        set_setting(_player.PUBLIC_DOMAIN_SETTING_KEY, bool(data.public_domain_fallback))
        _player.registry.clear_cache()  # resolution rules changed: drop cached hits/misses
    if data.cache_ttl_hours is not None:
        _player.registry.set_cache_ttl_hours(max(0.05, float(data.cache_ttl_hours)))
    if data.resolver_order is not None:
        _player.registry.set_order(list(data.resolver_order))
    if data.mediaflow_url is not None:
        try:
            set_setting(_mediaflow.URL_KEY, _mediaflow.normalize_url(data.mediaflow_url))
        except ValueError as e:
            raise HTTPException(status_code=400, detail=str(e))
    if data.mediaflow_api_password is not None:
        set_setting(_mediaflow.PASSWORD_KEY, data.mediaflow_api_password.strip())
    if data.mediaflow_enabled is not None:
        set_setting(_mediaflow.ENABLED_KEY, bool(data.mediaflow_enabled))
    if data.stremio_addon_url is not None:
        try:
            set_setting(_stremio.URL_KEY, _stremio.normalize_addon_url(data.stremio_addon_url))
        except ValueError as e:
            raise HTTPException(status_code=400, detail=str(e))
        _player.registry.clear_cache()
    if data.stremio_enabled is not None:
        set_setting(_stremio.ENABLED_KEY, bool(data.stremio_enabled))
        _player.registry.clear_cache()
    if data.vixsrc_enabled is not None:
        set_setting("vixsrc_enabled", bool(data.vixsrc_enabled))
        _player.registry.clear_cache()
    log_admin_action("UPDATE_SETTINGS", metadata=data.model_dump(exclude_none=True, exclude={"mediaflow_api_password"}))
    return {"success": True, **_mediaflow_settings(), **_stremio_settings()}


@app.post("/api/admin/settings/stremio/test")
async def admin_test_stremio(data: StremioTest, admin = Depends(get_current_admin)):
    url = data.stremio_addon_url if data.stremio_addon_url is not None else _stremio.get_config(get_setting)["url"]
    try:
        m = await _stremio.fetch_manifest(url)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except _stremio.StremioError as e:
        return {"ok": False, "detail": str(e)}
    supports_stream = "stream" in [r if isinstance(r, str) else (r or {}).get("name") for r in m.get("resources") or []]
    return {
        "ok": True,
        "detail": f"Addon raggiungibile: {m.get('name')} v{m.get('version', '?')}",
        "name": m.get("name"), "version": m.get("version"), "types": m.get("types") or [],
        "id_prefixes": m.get("idPrefixes") or [], "supports_stream": supports_stream,
    }


@app.get("/api/streams/{media_type}/{tmdb_id}")
async def list_streams(media_type: str, tmdb_id: int, season: Optional[int] = None, episode: Optional[int] = None, proxy: bool = True):
    """All playable streams for a title from the configured Stremio addon, optionally routed through MediaFlow."""
    if media_type not in ("movie", "tv"):
        raise HTTPException(status_code=400, detail="media_type deve essere 'movie' o 'tv'")
    if media_type == "tv" and (season is None or episode is None):
        raise HTTPException(status_code=400, detail="Per le serie TV indica season ed episode")
    cfg = _stremio.get_config(get_setting)
    if not cfg["enabled"]:
        raise HTTPException(status_code=404, detail="Nessun addon Stremio configurato")
    imdb_id = await _stremio.fetch_imdb_id(media_type, tmdb_id, db)
    if not imdb_id:
        raise HTTPException(status_code=404, detail="IMDb id non trovato per questo titolo")
    try:
        streams = await _stremio.fetch_streams(cfg["url"], media_type, imdb_id, season, episode)
    except _stremio.StremioError as e:
        raise HTTPException(status_code=404, detail=str(e))
    if not streams:
        raise HTTPException(status_code=404, detail="Nessuno stream disponibile per questo titolo")
    items = []
    for s in streams:
        payload = {"success": True, "stream": s["url"], "type": s["type"]}
        if s["headers"]:
            payload["headers"] = s["headers"]
        routed = _mediaflow.wrap_stream(payload, get_setting) if proxy else {**payload, "original_stream": s["url"], "proxied": False}
        items.append({
            "name": s["name"], "title": s["title"], "type": s["type"],
            "url": routed["stream"], "original_url": routed["original_stream"], "proxied": routed["proxied"],
            "not_web_ready": s["not_web_ready"],
        })
    return {"success": True, "media_type": media_type, "tmdb_id": tmdb_id, "imdb_id": imdb_id,
            "season": season, "episode": episode, "addon": cfg["url"], "count": len(items), "streams": items}


@app.post("/api/admin/settings/mediaflow/test")
async def admin_test_mediaflow(data: MediaflowTest, admin = Depends(get_current_admin)):
    cfg = _mediaflow.get_config(get_setting)
    url = data.mediaflow_url if data.mediaflow_url is not None else cfg["url"]
    password = data.mediaflow_api_password if data.mediaflow_api_password is not None else cfg["api_password"]
    try:
        return await _mediaflow.check_health(url, password)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def init_default_admin():
    """Migrate legacy admin_users into `users` with role, seed superadmins/plans/pages/menu (idempotent)."""
    import premium as _premium_seed
    _premium_seed.ctx["db"] = db
    _premium_seed.migrate(users, admin_users, menu_items, bcrypt)


def init_default_sections():
    """Seed the homepage rows once (trending, Top 10, latest, curated 'In arrivo'); always ensure the curated 'Novità' rows exist."""
    now = datetime.now(timezone.utc).isoformat()
    names = ["I titoli del momento", "Top 10 titoli oggi", "Aggiunti di recente", "In arrivo"] if sections.count_documents({}) == 0 else []
    names += ["Novità · Appena usciti", "Nuove stagioni · Nuovi episodi"]
    for name in names:
        if sections.find_one({"name": name}):
            continue
        t = next((s for s in AVAILABLE_SECTIONS if s["name"] == name), None)
        if t:
            sections.insert_one({**t, "apiString": t["section_type"], "mediaType": t.get("media_type", "movie"), "visible": True, "active": True, "order": sections.count_documents({}), "createdAt": now, "updatedAt": now})
...
def admin_login(data: AdminLogin):
    """Admin login: same `users` collection, authorized only via role."""
    admin = users.find_one({"email": data.email.lower()})
    if not admin or not bcrypt.checkpw(data.password.encode(), admin["password"].encode()):
        raise HTTPException(status_code=401, detail="Invalid credentials")
    if admin.get("role", "user") not in ("admin", "superadmin"):
        raise HTTPException(status_code=403, detail="Accesso riservato agli amministratori")
    if admin.get("banned"):
        raise HTTPException(status_code=403, detail="Account sospeso")
    users.update_one({"id": admin["id"]}, {"$set": {"last_login_at": datetime.now(timezone.utc).isoformat()}})
    log_admin_action("LOGIN", metadata={"email": admin["email"], "role": admin["role"]})
    return {
        "token": issue_user_token(admin),
        "email": admin["email"],
        "role": admin["role"],
        "must_reset_password": bool(admin.get("must_reset_password")),
        "expiresIn": JWT_EXPIRATION_HOURS * 7 * 3600
    }

@app.get("/api/admin/me")
def get_admin_profile(admin = Depends(get_current_admin)):
    """Get current admin profile"""
    return _premium.attach_status(dict(admin))
...
# ---- Native player: admin-managed stream sources (.m3u8 / .mp4) ----
@app.get("/api/admin/contents/{tmdb_id}/streams")
def get_content_streams(tmdb_id: int, admin = Depends(get_current_admin)):
    """List the stream sources configured by the admin for a title (movie: 1 row; tv: one row per episode)."""
    return {"tmdbId": tmdb_id, "streams": _player.list_stream_sources(tmdb_id)}

@app.put("/api/admin/contents/{tmdb_id}/stream")
def set_content_stream(tmdb_id: int, data: _player.StreamSourceUpdate, media_type: Optional[Literal["movie", "tv"]] = None, admin = Depends(get_current_admin)):
    """Set/replace the stream URL for a movie or a TV episode (season+episode required for tv)."""
    content = contents.find_one({"tmdbId": tmdb_id}, {"type": 1})
    resolved_type = media_type or (content or {}).get("type") or ("tv" if data.season is not None else "movie")
    doc = _player.upsert_stream_source(tmdb_id, resolved_type, data)
    log_admin_action("SET_STREAM_SOURCE", str(tmdb_id), {"media_type": resolved_type, "season": doc["season"], "episode": doc["episode"], "type": doc["type"]})
    return {"success": True, "stream": doc}

@app.delete("/api/admin/contents/{tmdb_id}/stream")
def delete_content_stream(tmdb_id: int, season: Optional[int] = None, episode: Optional[int] = None, admin = Depends(get_current_admin)):
    """Remove the admin stream source(s) for a title (optionally a single episode)."""
    deleted = _player.delete_stream_source(tmdb_id, season, episode)
    log_admin_action("DELETE_STREAM_SOURCE", str(tmdb_id), {"season": season, "episode": episode, "deleted": deleted})
...
AVAILABLE_SECTIONS = [
    ...
    {"name": "Novità · Appena usciti", "section_type": "new_releases", "media_type": "movie", "description": "Film curati da In Arrivo appena usciti"},
    {"name": "Nuove stagioni · Nuovi episodi", "section_type": "new_seasons", "media_type": "tv", "description": "Serie curate da In Arrivo appena uscite"},
...
def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)):
    """Verify JWT token and return user (any role)"""
    user = _decode_token_user(credentials)
    users.update_one({"id": user["id"]}, {"$set": {"last_seen_at": datetime.now(timezone.utc).isoformat()}})
    return user
...
def public_user(user: dict) -> dict:
    return _premium.attach_status({
        "id": user["id"], "email": user["email"], "name": user.get("name"), "profileImage": user.get("profileImage"),
        "role": user.get("role", "user"), "must_reset_password": bool(user.get("must_reset_password")), "premium": user.get("premium"),
    })
...
def get_user_profile(user = Depends(get_current_user)):
    """Get current user profile (with role + premium status)"""
    return _premium.attach_status(dict(user))
...
async def get_tmdb_upcoming(page: int = 1):
    """Upcoming: admin-curated "In arrivo" list when configured, otherwise TMDB future releases (IT region)."""
    import coming_soon as _cs
    curated = sorted((_cs.public_item(d) for d in db["coming_soon"].find({"status": "upcoming"})), key=lambda i: (_cs.sort_key(i), i.get("order", 0)))
    if curated:
        return {"items": await enrich_items(curated), "total": len(curated), "page": 1, "curated": True}
...
import coming_soon as _coming_soon  # noqa: E402
_support.register(app, db, get_current_user, get_current_admin, JWT_SECRET, log_admin_action, issue_user_token)
_premium.register(app, db, get_current_user, get_current_admin, log_admin_action, fetch_tmdb_data, enrich_items, _support.notify)
_coming_soon.register(app, db, get_current_admin, log_admin_action, fetch_tmdb_data, enrich_items, _support.notify)
_player.init_player(db, get_setting, set_setting)
app.include_router(_player.router)
